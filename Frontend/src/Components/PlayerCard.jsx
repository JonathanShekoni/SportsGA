...
const PlayerCard = ({name, points,rebounds,assists,fg, playerId, winnerPoints, winnerAssists, winnerRebounds}) => {
  ...
  return (
    <div>
      <img src={`https://cdn.nba.com/headshots/nba/latest/1040x760/${playerId}.png`} className="w-32 h-32 object-cover" />
      <p>{name.replace(/_/g, ' ')}</p>
      
      <div>
        <p className={`${winnerPoints === name ? 'text-green-400' : winnerPoints === 'tie' ? 'text-yellow-400' : 'text-red-400'} border-2 py-1 px-3 rounded h-10 flex items-center justify-center`}>{points}</p>
